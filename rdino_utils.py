...
def test_inference(model, criterion, data_loader, device, threshold=0.52):
    model.eval()
    y_true = []
    y_prob = []
    total_loss = 0.0
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            inputs, labels, png_names = data['image'], data['label'], data['png_name']  
...
def test_inference_test(model, criterion, data_loader, device, threshold):
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            # Loop over individual examples in the batch to compute misclassifications.
            for j, png_name in enumerate(png_names):
                true_val = labels[j].item()
                pred_val = cls_pred_bin[j].item()
                
                # If ground truth is 0 but prediction is 1 → false positive.
                if true_val == 0 and pred_val == 1:
                    false_positives.append(png_name)
                # If ground truth is 1 but prediction is 0 → false negative.
                elif true_val == 1 and pred_val == 0:
                    false_negatives.append(png_name)
    
    print('Test Loss (last batch):', loss) # loss.item()
